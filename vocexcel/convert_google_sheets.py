...
def split_and_tidy_gsheets(cell_value: str):
    ...
    if cell_value == '' or cell_value is None:
        return []
    return [x.strip() for x in cell_value.strip().split(",")] if cell_value is not None else []
...
def using_prefix_non_list_output(cell_value, prefix: dict, s: gspread.Worksheet, row: int):
    c = cell_value
    if c is not None:
...
def extract_concept(csheet: gspread.Worksheet, acsheet: gspread.Worksheet, prefix: dict):
        ...
        if rows[0] == "Concepts" or rows[0] == "Concept IRI*":
            ...
            try:
                c = models.Concept(
                    ...
                    provenance=rows[7],
                    home_vocab_uri=using_prefix_non_list_output(rows[8], prefix, csheet, row),
                    # additional concepts sheet page
                    related_match=using_prefix_list_output(split_and_tidy_gsheets(ac_val[row - 1][1]), prefix, acsheet, row),
                    close_match=using_prefix_list_output(split_and_tidy_gsheets(ac_val[row - 1][2]), prefix, acsheet, row),
                    exact_match=using_prefix_list_output(split_and_tidy_gsheets(ac_val[row - 1][3]), prefix, acsheet, row),
                    narrow_match=using_prefix_list_output(split_and_tidy_gsheets(ac_val[row - 1][4]), prefix, acsheet, row),
                    broad_match=using_prefix_list_output(split_and_tidy_gsheets(ac_val[row - 1][5]), prefix, acsheet, row)
                )
                row += 1
                concepts.append(c)
        ...
        if rows[0] == "Collections" or rows[0] == "Collection URI":
            ...
            try:
                c = models.Collection(
                    ...
                    pref_label=rows[1],
                    definition=rows[2],
                    members=using_prefix_list_output(split_and_tidy_gsheets(rows[3]), prefix, col_sheet, row),
                    provenance=rows[4]
                )
